        ...
        if address in cls.d:
            ...
            if isinstance(data,Commands):
                ...
                if isinstance(data,Key_D) and not Server.lista_players[Server.d[address]].AnimStart:
                    ...
                    if Server.lista_players[Server.d[address]].hitBox.right > 1360:
                        Server.lista_players[Server.d[address]].move_to(pygame.Vector2(0,Server.lista_players[Server.d[address]].y))
                    ...
                    Server.lista_players[Server.d[address]].move_collide([-20,0],[platform.getHitBox() for platform in Server.lista_platforms])
                    if Server.lista_players[Server.d[address]].hitBox.left < 0:
                        Server.lista_players[Server.d[address]].hitBox.right = 1360
                        Server.lista_players[Server.d[address]].move_to(pygame.Vector2(Server.lista_players[Server.d[address]].hitBox.x,Server.lista_players[Server.d[address]].y))
                ...
                for bullet in lista_bullet_to_remove:
                    ...
                    for platform in Server.lista_platforms:
                        if platform.collideKernel(bullet.getHitBox()):
                            Server.lista_bullet.remove(bullet)
                            if platform.isDestroyable:
                                Server.lista_platforms.remove(platform)
                            break
                ...
                for trigerred in Server.lista_trigerred:
                    found = False
                    for platform in Server.lista_platforms:
                        if type(platform) == Elevator:
                            elevator_to_stop = platform
                            found = True
                            break
                    if not found:
                        elevator_to_stop = None

                    trigerred.setObjectCollide(Server.lista_players[Server.d[address]])
                    trigerred.applyFunction((Server.lista_players[Server.d[address]],elevator_to_stop),data)		
            ...
            if data == "JOIN":
                if len(cls.d) < cls.MAX_NUMEBR_OF_PLAYERS:
                    if not (address in cls.d.keys()):
                        ...
                        cls.__send_udp((cls.d[address],cls.map_name),address)
                        cls.lista_time_to_respond[cls.d[address]] = time.time()
                        cls.lista_pickups = list(cls.map_dict['Pickup'])
